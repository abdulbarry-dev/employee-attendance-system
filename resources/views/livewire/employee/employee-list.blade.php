<div>
    ...
    <div class="mt-6 space-y-6">
        ...
        @if($employees->count() > 0)
            ...
            <div class="hidden md:block overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                                ...
                                <tr class="transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/70">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            <flux:button :href="route('employees.shifts', $employee)" wire:navigate size="sm" variant="ghost" icon="eye" tooltip="{{ __('View shifts') }}" />
                    ...
                    <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                        ...
                        <div class="flex items-start gap-3 mb-4">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="text-sm text-zinc-500 dark:text-zinc-400 mb-2 truncate">
                                    {{ $employee->email }}
                                </div>
                                @if($employee->is_banned)
                                    <flux:badge color="red" size="sm" icon="no-symbol">
                        ...
                        <!-- Contact Info -->
                        @if($employee->phone_number)
                            <div class="flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-300 mb-4 pb-4 border-b border-zinc-200 dark:border-zinc-700">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <flux:button :href="route('employees.shifts', $employee)" wire:navigate size="sm" variant="subtle" icon="eye" class="w-full">
                                {{ __('Shifts') }}
                            </flux:button>

                            <flux:button :href="route('employees.edit', $employee)" wire:navigate size="sm" variant="subtle" icon="pencil" class="w-full">
                                {{ __('Edit') }}
                            </flux:button>

                            @if($employee->is_banned)
                                <flux:button wire:click="unban({{ $employee->id }})" size="sm" variant="subtle" icon="check-circle" class="w-full">
                                    {{ __('Unban') }}
                                </flux:button>
                            @else
                                <flux:button wire:click="confirmBan({{ $employee->id }})" size="sm" variant="subtle" icon="no-symbol" class="w-full">
                                    {{ __('Ban') }}
                                </flux:button>
                            @endif
    ...
    <x-modal wire:model="showBanModal" maxWidth="md">
        <div class="p-6">
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <flux:button wire:click="ban" variant="primary">
                    {{ __('Suspend Employee') }}
                </flux:button>
            </div>
        </div>
